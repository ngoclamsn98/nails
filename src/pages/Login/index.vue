<template>
  <div class="w-full relative h-full flex items-center bg-sky-200">
    ...
    <form
      @submit="onSubmit"
      class="gap-y-[10px] flex flex-col w-[80%] m-auto h-[485px] justify-end"
    >
      ...
      <div class="flex justify-between px-5 mt-[10px] items-center">
        <span>Đăng nhập</span>
        <button class="w-[50px] h-[50px] rounded-full bg-sky-600">-></button>
      </div>
    </form>
  </div>
</template>
...
const handleSubmitForward = handleSubmit((values) => {
  alert(JSON.stringify(values, null, 2));
});

const onSubmit = (e) => {
  handleNextFocus(e, handleSubmitForward);
};
</script>
